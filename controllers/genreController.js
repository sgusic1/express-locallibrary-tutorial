       ...
       //Extract the validation errors from a request.
        ...
        if (!errors.isEmpty()) {
            ...
            if (genreExists) {
                ...
                //Genre exists, redisrect to its detail page.
...
exports.genre_delete_get = asyncHandler(async (req, res, next) => {
    const [genre, allGenreBooks] = await Promise.all([
        Genre.findById(req.params.id).exec(),
        Book.find({genre: req.params.id}, "title genre")
            .populate("author")
            .populate("genre")
            .exec(),
    ]);
...
exports.genre_update_post = [
    ...
    asyncHandler(async (req, res, next) => {
       ...
       if (!errors.isEmpty()) {
           ...
           const genreExists = await Genre.findOne({ name: genre.name}).exec();
           if (genreExists) {
               res.redirect(genreExists.url);
               return
           } else {
               const updateGenre = await Genre.findByIdAndUpdate(genre._id, genre, {});
               res.redirect(genre.url);
           }
